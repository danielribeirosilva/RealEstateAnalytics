...
def getAllActiveRefNum():
    con = connectToDefaultDatabase()
    cur = con.cursor()
    try:
        query = "SELECT ref_num FROM active GROUP BY ref_num"
        cur.execute(query)
        all_ref_num = [row[0] for row in cur.fetchall()]
        return all_ref_num
    except MySQLdb.Error, e:
...
def updateDbWithActiveProperties(new_active, no_longer_active):
    con = connectToDefaultDatabase()
    cur = con.cursor()
    #add ref_num that is now active
    for ref_num in new_active:
        query = "INSERT INTO active(ref_num) VALUES("+str(ref_num)+")"
        try:
           cur.execute(query)
           con.commit()
        except:
           con.rollback()
    #remove no longer active ref_nums
    for ref_num in no_longer_active:
        query = "DELETE FROM active WHERE ref_num="+str(ref_num)
        try:
           cur.execute(query)
           con.commit()
        except:
...
# mysqldump -u crawler -pcrawler real_estate_analytics --compact --no-data > db_dump.sql